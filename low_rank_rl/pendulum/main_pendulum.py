...
from utils import Mapper, MhRL,PendulumEnv



def parse_args():
    parser = argparse.ArgumentParser(description='RL task: Pendulum')

    parser.add_argument('--seed', type=int, default=1)

    # algorithm parameters
    parser.add_argument('--alg', type=str, default='')
    parser.add_argument('--episodes', type=int, default=20000, help='K')
...
def wandb_train(config=None):
    ...
    with wandb.init(config=config, save_code=True):
        ...
        if args.alg == 'Mhlearning':
            parameters_file = "low_rank_rl/pendulum/experiments/exp_1_Mh_learning.json"
...
if __name__ == '__main__':
    ...
    sweep_id = wandb.sweep(sweep_config, project="Test Low-rank Pendulum")
